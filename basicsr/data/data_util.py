...
def paired_paths_from_folder1(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    for idx in range(len(gt_paths)):
        ...
        basename_input, ext_input = osp.splitext(osp.basename(gt_path))
        input_dir = osp.join(input_folder, basename)
        # assert input_name in input_paths, (f'{input_name} is not in '
        #                                    f'{input_key}_paths.')
        gt_path = osp.join(gt_folder, gt_path)
        for i in scandir(input_dir):
            input_path = osp.join(input_dir, i)
            paths.append(
                dict([(f'{input_key}_path', input_path),
                      (f'{gt_key}_path', gt_path),]))
    return paths
...
def paired_paths_from_folder2(folders, keys, filename_tmpl):
    ...
    assert len(keys) >= 2, (
        'The len of keys should be 2 with [input_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder, label_folder = folders
    input_key, gt_key, label_key = keys

    input_paths = list(scandir(input_folder))
    gt_paths = list(scandir(gt_folder))
    label_paths = list(scandir(label_folder))
    ...
    for idx in range(len(gt_paths)):
        ...
        _, ext_label = osp.splitext(osp.basename(label_path))
        input_name = f'{filename_tmpl.format(basename_input)}{ext_input}'
        input_path = osp.join(input_folder, input_name)
        label_name = f'{filename_tmpl.format(basename_input)}{ext_label}'
        label_path = osp.join(label_folder, label_name)
        assert input_name in input_paths, (f'{input_name} is not in '
                                           f'{input_key}_paths.')
        gt_path = osp.join(gt_folder, gt_path)
        paths.append(
