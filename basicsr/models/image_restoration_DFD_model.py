...
class ImageRestorationDFDModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRestorationDFDModel, self).__init__(opt)
        if opt['datasets'].get('train', False):
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        for i, (lq, gt) in enumerate(zip(lq_list, gt_list)):
            ...
            # fft loss
            if self.cri_fft:
                l_fft = self.cri_fft(preds[-1], gt - lq)
                l_total += l_fft
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                # in_tensor1 = F.pad(self.lq[1][i:j], (0, w_n, 0, h_n), mode='reflect')
    ...
    def get_current_visuals(self):
        ...
        # out_dict['result'] = self.rgb_to_grayscale(self.lq).detach().cpu()
        if hasattr(self, 'gt'):
            out_dict['gt'] = (self.gt - self.lq).detach().cpu()
        return out_dict
    ...
    def print_model_params(self):
        # 如果模型是 DistributedDataParallel，需要访问其 module 属性
        if isinstance(self.net_g, nn.parallel.DistributedDataParallel):
            model = self.net_g.module

        # 打印模型中所有参数
        for name, param in model.named_parameters():
            if name == 'v':
                with open("output.txt", "a") as file:
                    file.write(str(param.data.tolist()[0]))
                    file.write("\n")