...
class ImageRestorationS1Model(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageRestorationS1Model, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.opt['train'].get('mixup', False):
            self.mixup_aug()
        preds, _ = self.net_g(torch.cat([self.lq, self.gt - self.lq], dim=0))
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                in_tensor1 = F.pad(self.gt[i:j] - self.lq[i:j], (0, w_n, 0, h_n), mode='reflect')
                pred, _ = self.net_g(torch.cat([in_tensor0, in_tensor1],dim=0))
                pred = pred[:, :, :h, :w]
